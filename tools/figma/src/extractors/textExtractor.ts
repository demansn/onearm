...
/**
 * Extract TextBlock properties with elements array format
 */
export function extractTextBlockProps(node: AbstractNode): any {
  if (node.type !== 'TEXT') {
    return {};
  }
  ...
  // Add size property to style for TextBlock
  style.size = {
    width: Math.round(node.width),
    height: Math.round(node.height)
  };

  // Add default vAlign value for TextBlock
  style.vAlign = 'middle';
