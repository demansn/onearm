# Pixi Layout v1 — Core Reader (Reference)

A minimal reference implementation of the Pixi Layout v1 **Core profile** reader, for any Pixi.js application.

Specification: [`docs/pixi-layout-v1.md`](../../docs/pixi-layout-v1.md) (Part I).
...
- `test/run-fixtures.ts` — walks the conformance fixtures at `docs/fixtures/pixi-layout-v1/` and asserts that valid docs validate and invalid docs are rejected.
...
```bash
cd reference/pixi-layout-reader
npm install
npm test           # validates against docs/fixtures/pixi-layout-v1/
npm run build
...
```
reference/pixi-layout-reader/
├── package.json
├── tsconfig.json
├── index.html
├── README.md
├── src/
│   ├── types.ts           # Vendored subset of the Pixi Layout v1 TS schema
...
The canonical TypeScript schema lives at `tools/figma/src/schema/pixi-layout-v1.schema.ts` and is kept in sync with `src/types.ts` here by hand. The vendored copy exists so the reference stays self-contained; if the spec changes, update both.
