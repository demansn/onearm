...
/**
 * Generate asset manifest by scanning assets/ directories.
 *
 * Signatures:
 *   generateManifest(gameRoot: string)
 *     Legacy: scans gameRoot/assets/, distImgDir=gameRoot/dist/assets/img
 *     Writes resources-manifest.js to gameRoot/src/configs/ (side-effect)
 *     Returns the manifest object.
 *
 *   generateManifest({ sources: string[], distImgDir: string })
 *     New: scans each dir in sources (merged, last wins), returns manifest object.
 ...
 *   img/{name}{tps}/          → {name} bundle if it exists, else main bundle, spritesheet
 ...
 */
export function generateManifest(arg) {
    let sources, distImgDir, legacyGameRoot;

    if (typeof arg === "string" || arg == null) {
        // Legacy: generateManifest(gameRoot)
        legacyGameRoot = arg ?? findGameRoot();
        sources = [path.join(legacyGameRoot, "assets")];
        distImgDir = path.join(legacyGameRoot, "dist", "assets", "img");
    } else {
        ({ sources, distImgDir } = arg);
    }
    ...
    // Manifest structure is determined by the base (first) source.
    // Overlay sources only replace file content at the same paths — no new aliases.
    const baseAssetsDir = sources[0];
    addStaticEntries(baseAssetsDir, bundles);
    discoverFonts(baseAssetsDir, bundles);
    discoverBitmapFonts(baseAssetsDir, bundles);
    discoverSpine(baseAssetsDir, bundles);
    discoverSpritesheets(baseAssetsDir, bundles);
    discoverSounds(baseAssetsDir, bundles);
    discoverPlinko(baseAssetsDir, bundles);

    // Images reference dist output — the merged AssetPack output manifest
    discoverImages(baseAssetsDir, bundles, distImgDir);

    const manifest = {
        bundles: Array.from(bundles.entries()).map(([name, assets]) => ({ name, assets })),
    };

    if (legacyGameRoot != null) {
        // Legacy: write resources-manifest.js as a side-effect
        const code =
            "// Auto-generated by generate-manifest.js — do not edit manually\n" +
            `export const manifest = ${JSON.stringify(manifest, null, 4)};\n`;

        const outputPath = path.join(legacyGameRoot, "src/configs/resources-manifest.js");
