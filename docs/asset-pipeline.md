...
### Spine Manifest для Figma

Команда `generate-spine` сканирует `assets/spine/` и генерирует `spine-manifest.json` с метаданными всех скелетов: размеры, списки анимаций, скинов. Для JSON-скелетов вычисляются **per-animation размеры** на основе attachment bounds каждой анимации.

```bash
# Генерация манифеста (по умолчанию → output/spine-manifest.json)
node bin/onearm-figma.js generate-spine
node bin/onearm-figma.js generate-spine --output=path/to/manifest.json
```
...
- `npm run spine-preview` — Spine Previewer: интерактивный инструмент для просмотра spine-анимаций (см. `docs/spine-previewer.md`)
- `node bin/onearm-figma.js generate-spine` — генерация Spine-манифеста для Figma-плагина (per-animation размеры, скины, анимации)
