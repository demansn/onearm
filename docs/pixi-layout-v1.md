...
Pixi Layout is a portable layout configuration format for Pixi.js-based applications. It specifies how trees of visual nodes are serialized and consumed, without encoding engine-specific runtime behavior.
...
### Goals
- Simple: a minimum-subset Core reader (container / sprite / text, no masks, no custom types) fits in under 50 lines of TypeScript; a full Core reader covering every intrinsic type plus masks fits in roughly 100 lines. See the reference implementation at `reference/pixi-layout-reader/` for the latter.
...
Every Pixi Layout document has this envelope:
...
|---|---|---:|---|
| `format` | string | yes | MUST be `"pixi-layout"` |
...
- **SVG profiles** (Tiny / Basic / Full) — the W3C precedent for explicit layered conformance tiers in a single format. Pixi Layout's Core / Library / Scene profiles are directly inspired by this pattern.
